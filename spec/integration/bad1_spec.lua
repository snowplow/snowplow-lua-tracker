...
describe("Integration tests with bad config", function()
  it("should throw an error on a bad configuration option", function()
    local t = snowplow.new_tracker("d3rkrsqld9gmqf.cloudfront.net")
  ...
  it("should throw an error without impacting another tracker", function()
    local t1 = snowplow.new_tracker("d3rkrsqld9gmqf.cloudfront.net")
    t1:platform("tv")
    ...
    local t2 = snowplow.new_tracker("cc-endpoint2.beanstalky.com")
    ...
    assert.has_no.errors(function()
      t1:track_struct_event("hud", "save", nil, nil, nil, 1368725287)
    end)
  end)
  ...
  it("should be reusable if the error is caught", function()
    ...
    assert.has_no.errors(function()
      t1:track_screen_view("Test", "23", 1368725287)
    end)
  end)
end)
